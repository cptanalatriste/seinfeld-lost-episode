...
from lost_episode.data_utils import create_int_to_word_map, create_word_to_int_map, create_punctuation_map, create_batch_dataloader
...
class TestDataUtils(unittest.TestCase):
    ...
    def test_batch_dataloader(self):
        ...
        self.assertTrue(torch.all(first_batch_features.eq(torch.IntTensor([1, 2, 3, 4]))))
        self.assertEqual(5, first_batch_target.item())

        second_batch_features, second_batch_target = next(data_iterator)
        self.assertTrue(torch.all(second_batch_features.eq(torch.IntTensor([2, 3, 4, 5]))))
